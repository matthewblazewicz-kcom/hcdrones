...
@NoArgsConstructor
@AllArgsConstructor
public class PerEndpointSolution {
    ...
    public List<CacheServerStatus> solveIt() {
        ...
        for (int serverId = 0; serverId < service.getNumberOfServers(); serverId++) {
            TreeSet<VideoWithIndex> sortedVideosForEndpoint = new TreeSet<>();
            for (int videoIndex = 0; videoIndex < service.getNumberOfVideos(); videoIndex++) {
                sortedVideosForEndpoint.add(new VideoWithIndex(videoIndex,
            ...
            cachedVideos = "";
            usedCapacity = 0;
            for (VideoWithIndex vid : sortedVideosForEndpoint) {
